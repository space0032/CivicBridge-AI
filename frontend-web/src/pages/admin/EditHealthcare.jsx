...
const EditHealthcare = () => {
        ...
        const fetchFacility = async () => {
            try {
                ...
                setFormData({
                    ...
                    freeServices: facility.freeServices || false,
                    isActive: facility.isActive !== undefined ? facility.isActive : true
                });
            } catch (err) {
                setError('Failed to fetch facility details. ' + (err.response?.data?.message || err.message));
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setSubmitting(true);
            setError(null);
            await healthcareService.update(id, formData);
            navigate('/admin');
        } catch (err) {
        ...
        <div style={styles.container}>
            <div style={styles.card}>
                ...
                <div style={styles.header}>
                    <h1 style={styles.title}>Edit Healthcare Facility</h1>
                    <p style={styles.subtitle}>Modify the details of the healthcare facility.</p>
                </div>
                ...
                <form onSubmit={handleSubmit} style={styles.form}>
                    ...
                    <div style={styles.row}>
                        <div style={{ ...styles.formGroup, flex: 1 }}>
                            ...
                            <input id="latitude" type="number" step="any" name="latitude" value={formData.latitude} onChange={handleChange} style={styles.input} />
                        </div>
                        <div style={{ ...styles.formGroup, flex: 1 }}>
                            <label htmlFor="longitude" style={styles.label}>Longitude</label>
                            <input id="longitude" type="number" step="any" name="longitude" value={formData.longitude} onChange={handleChange} style={styles.input} />
                        </div>
                    </div>
                    ...
                    <div style={styles.formGroup}>
                        ...
                        <input id="operatingHours" type="text" name="operatingHours" value={formData.operatingHours} onChange={handleChange} style={styles.input} />
                    </div>

                    <div style={styles.formGroup}>
                        <label htmlFor="services" style={styles.label}>Services (comma separated)</label>
                        <textarea id="services" name="services" value={formData.services} onChange={handleChange} style={styles.input}></textarea>
                    </div>
